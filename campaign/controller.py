...
class SearchController(QtViewController):
    ...
    def bind(self, view):
        view.searchEdit.textChanged.connect(self.on_search_text_changed)
        view.searchEdit.returnPressed.connect(self.on_search_requested)
...
class CampaignController(QtViewController):
    ...
    def __init__(self, delphi, campaign, archive_meta=None):
        ...
        self.dice_controller = None
        self.namegen_controller = None
        self.asset_tree_model = TreeModel(TreeNode())
    ...
    def init_subcontrollers(self):
        ...
        self.search_controller = SearchController(self.delphi,
                                                  SearchCompleter())
        self.namegen_controller = NameGenController()
        self.dice_controller = DiceController(self.campaign.dice)
    ...
    def bind(self, v):
        ...
        for controller in self._subcontrollers_using_campaign_window():
            controller.bind(self.view)

    def init_asset_tree(self):
        root = FixedNode(*[controller.tree_node for controller in
                           self._subcontrollers_with_tree_nodes()])
        self.asset_tree_model = TreeModel(root)
    ...
    def _subcontrollers(self):
        return [self.map_controller, self.session_controller,
                ...
                self.namegen_controller, self.dice_controller]

    def _subcontrollers_using_campaign_window(self):
        return [self.map_controller, self.session_controller,
                self.player_controller, self.note_controller]

    def _subcontrollers_with_tree_nodes(self):
        return [con for con in self._subcontrollers()
                if hasattr(con, "tree_node")]
